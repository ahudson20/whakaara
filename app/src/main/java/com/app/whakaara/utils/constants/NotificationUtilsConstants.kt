...
object NotificationUtilsConstants {
    ...
    val INTENT_SCHEDULE_ALARM_PERMISSION =
        Intent().apply { action = Settings.ACTION_REQUEST_SCHEDULE_EXACT_ALARM }
    private val INTENT_APP_SETTINGS =
        Intent(Settings.ACTION_APPLICATION_DETAILS_SETTINGS).apply { addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }
    val INTENT_DATE_SETTINGS =
        Intent(Settings.ACTION_DATE_SETTINGS).apply { addFlags(Intent.FLAG_ACTIVITY_NEW_TASK) }
